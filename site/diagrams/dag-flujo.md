...
| El closer no puede marcar `done` sin commit + push + runtime cleanup + worktree cleanup | `enforce_closer_done_guardrail` en hook |
...
./scripts/validate-orchestrator-schemas.sh --json
./scripts/run-golden-e2e.sh --json
```

These checks validate the orquestador, not the product. ChatGPT still fills only five source-of-truth docs.


## Runtime paralelo por slice

`/next-wave` no sólo prepara el worktree: también exporta `COMPOSE_PROJECT_NAME=<compose_project>` y ejecuta `allocate-slice-ports.sh` para generar `orchestrator-state/dev-ports/<compose_project>.env`. Docker Compose project names aíslan nombres/redes/volúmenes; los puertos host se aíslan con `CLAUDE_FRONTEND_PORT`, `CLAUDE_BACKEND_PORT`, `CLAUDE_DB_PORT`, etc., detectando primero qué puertos ya están ocupados.
