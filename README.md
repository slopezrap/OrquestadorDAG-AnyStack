...
ChatGPT Pro rellena templates
  ...
  -> claude --agent main-orchestrator --permission-mode bypassPermissions "/next-slice <TASK_ID>" implementa, valida/testea y encadena /verify-slice automáticamente
  -> verified_pending_close queda listo; el usuario decide cuándo ejecutar /closer manual
  -> /closer manual genera report + sync baseline + lifecycle-event + commit + configured Git workflow + limpia worktrees
...
**Framework hardening**: sigues entregando sólo cinco docs a ChatGPT. Los JSON schemas, `orchestrator-doctor` y `examples/golden-real-app/` son infraestructura interna del framework: validan artefactos derivados, detectan drift y prueban un flujo E2E real. No son un sexto source-of-truth ni una plantilla de stack; son validadores internos del framework.

Contrato operativo actual: ChatGPT sigue rellenando solo los 5 documentos source-of-truth. Los schemas, `orchestrator-doctor` y la golden app son validadores internos. La golden app no fija el stack: cualquier stack declarado en `STACK_PROFILE.yaml` es válido si demuestra datos reales/proporcionados, UI/controles reales, persistencia, reglas DR-* y logs limpios.

Nota DAG: la política actual elimina los topes artificiales de tamaño de phase/step. `--strict` falla por errores estructurales del DAG, drift de vistas o dependencias inválidas; la granularidad se decide por trazabilidad, ownership, `Write set`, `Conflict group` y verificación real.
...
No hagas `unset` al terminar `/next-slice` si vas a cerrar esa misma task: el flujo normal ya verifica automáticamente al final de `/next-slice`. Si hubo interrupción, `/clear` o verify stale, conserva el mismo `CLAUDE_ACTIVE_TASK_ID` y `CLAUDE_TASK_PACK`, y relanza la verificación recovery de esa misma slice:

```bash
claude --agent main-orchestrator --permission-mode bypassPermissions "/verify-slice P02-S03-T001"
claude --agent main-orchestrator --permission-mode bypassPermissions "/closer P02-S03-T001"
```

Cuando `/closer` haya dejado la task cerrada, limpia el terminal antes de reclamar otra task:
...
planner
  ...
  -> verify-slice automático si validator/tester están verdes
  -> verified_pending_close si el gate humano-real pasa
```

La ruta normal después es manual y explícita:

```text
/closer P02-S03-T001
```

Si interrumpiste el flujo, hiciste `/clear` antes del verify o el checker detecta verify stale, usa:
...
`/verify-slice` hace hard reset y carga datos reales/proporcionados del `Verification Data Contract`, reproducción humana front→back→DB y, si queda verified, deja la slice lista para `/closer`. Si encuentra hallazgos menores y dentro del `Write set`, debe llamar a `debugger`, repetir `validator ‖ tester` y relanzar `/verify-slice`; si el hallazgo es mayor o fuera de alcance, registra follow-up formal.

Antes de permitir `/closer`, `/verify-slice` valida que el handoff no esté roto:

```bash
./scripts/check-handoff-contract.sh <TASK_ID> --require-ready-for-close --require-verify-slice
```

Si la task es de pantalla, UX, journey, gate visual o contiene `VISUAL_CONTRACT_CHECK`, `/verify-slice` ejecuta además el reviewer info-only `screen-journey-reviewer` antes de dejarla lista para `/closer` y exige:
...
cleanup-slice-runtime --task <TASK_ID> --apply  # Docker/Rancher runtime de la slice
slice-clean
cleanup-worktrees --apply --task <TASK_ID> --schedule-active
hook marca done solo si REPORT/GIT/PUSH/RUNTIME/WORKTREES/BASELINE_SYNC son yes
```
...
./scripts/register-followup-task.sh propose \
  ...
  --description "Verify necesita estado empty con datos reales/proporcionados persistidos" \
  ...
  --verify "/verify-slice observa estado empty con cuenta de prueba real/proporcionada persistida"
```
...
### Stack Docker aislado por slice

`./scripts/next-wave.sh` exporta `COMPOSE_PROJECT_NAME` derivado del `TASK_ID` de la slice, por ejemplo `P01-S02-T003` -> `p01-s02-t003`, y ejecuta el asignador de puertos por slice. La unidad de aislamiento es la slice, no el worktree: `docker compose -p <compose_project>` aísla nombres/redes/volúmenes, y `CLAUDE_FRONTEND_PORT`/`CLAUDE_BACKEND_PORT`/`CLAUDE_DB_PORT` evitan colisiones de puertos host entre worktrees paralelos.

Si necesitas un nombre distinto por una razón operativa, exporta `COMPOSE_PROJECT_NAME=<otro>` ANTES de pegar el bloque copy/paste; si necesitas puertos fijos, exporta `CLAUDE_FRONTEND_PORT`/`CLAUDE_BACKEND_PORT`/etc. antes del allocator. Si no, deja que el orquestador detecte puertos ocupados y asigne libres.

Al cerrar, `/closer` debe ejecutar también:

```bash
./scripts/cleanup-slice-runtime.sh --task <TASK_ID> --apply --strict
```

Ese helper limpia sólo recursos de la slice: `docker compose -p <compose_project> down -v --remove-orphans --rmi local`, objetos con label `com.docker.compose.project=<task_slug>`, reservas `orchestrator-state/dev-ports/<compose_project>.*` y el cleanup Rancher declarado en `STACK_PROFILE.yaml` si existe. No ejecuta `docker system prune` ni borra imágenes compartidas arbitrarias.
...
claude --agent main-orchestrator --permission-mode bypassPermissions "/next-slice <TASK_ID>"
                                  ejecuta implementación + verify automático; no cierra
/verify-slice <TASK_ID>            recovery/gate humano; deja verified_pending_close si pasa
/closer <TASK_ID>                  cierre manual: report + baseline + commit + Git workflow + runtime/worktree cleanup
/auto-verify-slice <TASK_ID>       verificación automática solo low+auto; deja lista para /closer
...
- [Pages site (live)](https://slopezrap.github.io/OrquestadorDAG-AnyStack/) — overview, negocio, técnico, comandos, DAG, outcomes y stack/UX.
...
- [Pages site (live)](https://slopezrap.github.io/OrquestadorDAG-AnyStack/) servido desde `site/html-site/` vía GitHub Actions.
...
- **Phase** = milestone o módulo de producto con visión completa; no tiene tope artificial de slices.
- **Step** = lane coherente dentro de la phase: pantalla/journey lane, módulo de dominio, foundation lane o contrato API que alimenta una pantalla nombrada; no tiene tope artificial de tasks.
...
- No dividas ni fusiones por números. Divide cuando mezcle lanes no relacionadas, toque write sets incompatibles, bloquee paralelismo real o pierda trazabilidad de producto.
...
- Los templates deben sustituir todos los ejemplos por el dominio real de la app, declarar todas las slices necesarias y usar datos reales/proporcionados; si faltan datos, bloquea o registra follow-up.
...
### Verify-slice human web/mobile MCP gate

`/verify-slice` delegates to `slice-verifier`, which must perform hard reset, load real/provided verification data, exercise the app through a **usable** visual MCP, observe front/back/DB/worker logs, write evidence, and append `## verify-slice`. For web/browser surfaces, the priority is Chrome DevTools MCP first, Claude-in-Chrome second fallback, Agent360 Browser MCP (`browser-mcp`) third fallback. For Flutter mobile surfaces, browser MCP is not enough: use Dart/Flutter MCP with `MCP_CLIENT: dart|flutter|flutter-driver`, `VISUAL_CHECK_METHOD: simulator|emulator|device`, and `MCP_BROWSER: not_applicable:flutter_mobile`. `slice-verifier` has `maxTurns: 130` for visual MCP work; keep the global spawn budget at 20 because it counts subagents, not MCP tool calls. Listed tools are not enough: the agent must make a short health call before relying on an MCP. If Chrome DevTools MCP is locked by a stale/active Chrome profile, use `bash scripts/chrome-mcp-doctor.sh || true` plus `scripts/chrome-devtools-isolated-session.sh --task <TASK_ID>` for diagnostics/isolation before trying fallbacks. If no accepted MCP is usable it blocks with `browser_mcp_unavailable` or `flutter_mobile_mcp_unavailable`; it must not close via API-only fallback. A broken unused MCP does not invalidate a verification already completed through another accepted MCP.

`slice-verifier` has a slightly larger local budget (`maxTurns: 130`) because visual MCP verification is tool-heavy. The global per-slice spawn budget remains 20. Near budget exhaustion the verifier must persist a final blocked handoff with `BLOCKER_REASON: mcp_budget_exhausted_or_scope_too_large`, not leave a partial run.
...
For web setup and isolation details see `docs/guides/MCP_BROWSER_VERIFY.md`. Short web policy: use Chrome DevTools MCP first for normal browser verify work, including MFA/2FA when a visible isolated/per-task Chrome can be used; use Claude-in-Chrome as the second fallback; use Agent360 Browser MCP (`browser-mcp`) as the third fallback for real-session/human-in-the-loop cases when the first two are unusable. Short mobile policy: Flutter mobile verification uses Dart/Flutter MCP with a real simulator/emulator/device; do not replace it with a browser-only session.

The handoff contract for human `VERIFY_OUTCOME: verified` is intentionally strict: it must include either accepted web MCP evidence (`MCP_BROWSER`) or accepted mobile MCP evidence (`MCP_CLIENT` + `VISUAL_CHECK_METHOD`), plus `DATA_CONTRACT_ROWS`, `DATA_SETUP`, `PERSISTED_DATA_OBSERVED`, `FLOWS_TESTED` and `EVIDENCE`. The only no-MCP exception is `VERIFY_MODE: auto` with `RISK_LEVEL: low` and deterministic command evidence. This prevents a thin/partial `verified` block from letting `/closer` mutate the DAG to `done`.
...
- Shared frontend/mobile/auth/chat/router/error files require full human-real `/verify-slice` evidence before `/closer`: browser MCP for web/browser surfaces, Dart/Flutter MCP for Flutter mobile. `/next-slice` auto-enters verify, but shared frontend/mobile/auth/chat/router/error files require the full human-real gate; do not use the narrow `/auto-verify-slice` helper for them.
...
## Production-real verification / logs
...
- Si hay Docker Compose, usa aislamiento por slice con `docker compose -p <compose_project>` y puertos host asignados por slice; el helper canónico es `./scripts/check-runtime-logs.sh --task <TASK_ID> --mode hard-reset`, que reconstruye desde el worktree activo tras generar `orchestrator-state/dev-ports/<compose_project>.env`.
...
## Framework doctor and golden E2E

This repository includes a framework-level health check and a real golden app fixture. Use them when modifying the orquestador itself:
...
`orchestrator-doctor` validates the static contract: agents, commands, schemas, source-of-truth state, model aliases, stale slice caps, runtime reality fields and golden fixture presence. `run-golden-e2e.sh` runs a real HTTP + SQLite app with provided data, UI controls, domain-rule verification and runtime-log scanning, then bootstraps its five source-of-truth docs and verifies the generated DAG/next-wave output.

The purpose is production discipline: no stub data, no invented fixtures, no fake PDF/LLM extraction, no unchecked worker logs, and no closing a slice just because code compiled.
