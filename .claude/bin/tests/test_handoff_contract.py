...
def _run_with_production_observability(tmp_path: Path, handoff_text: str, task_id: str = "P00-S01-T001"):
    handoffs = tmp_path / "orchestrator-state" / "tasks" / "handoffs"
...
def _production_verify_extra(task_id: str) -> str:
    return f"""- REAL_USER_VERIFIED: yes
- NO_STUB_DATA: yes
- RUNTIME_LOGS_REVIEWED: browser/front/back/DB/worker/Docker/Rancher
- RANCHER_WORKER_LOGS_REVIEWED: not_applicable:no_rancher_worker_in_stack
- ERROR_LOGS_STATUS: clean
- DOCKER_COMPOSE_PROJECT: p00-s01-t001
- DOCKER_PORTS_ALLOCATED: not_applicable:no_docker_host_ports_published
- NO_STUB_DATA_USED: yes
- REAL_DATA_SOURCE: VDC-001 user-provided fixture
- HUMAN_REPRODUCTION: yes
- BUTTONS_AND_CONTROLS_CHECKED: not_applicable:backend-only deterministic task
- RUNTIME_LOGS_CHECKED: yes
- RANCHER_WORKER_LOGS_CHECKED: not_applicable:no_rancher_worker_in_stack
...
def test_handoff_contract_requires_production_observability_fields_when_flagged(tmp_path: Path) -> None:
    task_id = "P00-S01-T001"
    missing = _run_with_production_observability(
        tmp_path / "missing",
        f"""
# Task Handoff — {task_id}
...
""",
    ...
    assert "NO_STUB_DATA_USED" in missing.stdout
    assert "RUNTIME_LOG_ERRORS" in missing.stdout

    ok = _run_with_production_observability(
        tmp_path / "ok",
        f"""
# Task Handoff — {task_id}
...
{_verified_verify_block(task_id, extra=_production_verify_extra(task_id))}
""",
        task_id,
    )
