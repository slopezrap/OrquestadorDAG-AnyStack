...
def test_verify_slice_repeats_dag_mode_and_passes_dag_context_to_spawns() -> None:
    ...
    assert "NO invoca closer" in text
    assert "/closer <TASK_ID>" in text
    assert "CLAUDE_TASK_PACK=orchestrator-state/tasks/task-packs/<TASK_ID>.md" in text
    assert "verified_pending_close" in text
...
def test_closer_command_uses_configured_git_workflow_not_direct_push_wording() -> None:
    text = (ROOT / ".claude/commands/closer.md").read_text(encoding="utf-8")
