...
def test_templates_explain_phase_step_slice_granularity_without_artificial_caps() -> None:
    for profile in ["minimal", "large-with-base", "large-without-base"]:
        ...
        assert "Granularidad sana" in text
        assert "topes artificiales" in text.lower() or profile == "minimal"
        assert "phase <=20 slices" not in text
        assert "<=15 máximo" not in text
        assert "11-12 slices" not in text
