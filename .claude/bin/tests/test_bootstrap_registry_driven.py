...
class BootstrapRuntimePreservationTests(unittest.TestCase):
    ...
    def test_refresh_does_not_block_when_only_build_state_changes(self):
        td, root = self._root()
        try:
            with self._with_root(root) as (boot_mod, common_mod):
                self.assertTrue(boot_mod.generate_artifacts()["ok"])
                ...
                first["status"] = "ready_for_close"
                first["last_updated_by"] = "tester"
                common_mod.save_registry(registry)

                sot = root / "docs/source-of-truth"
                changed = self.CHECKLIST.replace("| planned | low |", "| done | low |", 1)
                (sot / "APP_IMPLEMENTATION_CHECKLIST.md").write_text(changed, encoding="utf-8")

                result = boot_mod.generate_artifacts()
                self.assertTrue(result["ok"])
                task = {t["id"]: t for t in common_mod.load_registry()["tasks"]}["P00-S01-T001"]
                self.assertEqual(task["source_fingerprint"], old_fp)
                self.assertEqual(task["status"], "ready_for_close")
                self.assertNotEqual(task.get("blocked_reason"), "source_of_truth_changed_after_runtime_state")
        finally:
            td.cleanup()
