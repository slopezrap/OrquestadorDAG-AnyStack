...
- **Rol:** "Implements exactly one approved DAG task pack at a time. Use after planner returns CONTEXT_READY: yes.".
...
Si el task pack declara `Domain rule refs`, trata esas reglas `DR-*` como acceptance de primera clase: localiza su definición en `instrucciones.md`, su implementación esperada en el TECHNICAL_GUIDE y prueba tanto happy path como fallos de dominio. Si falta una regla necesaria o una referencia no existe, no inventes dominio: bloquea o abre follow-up de source-of-truth.
