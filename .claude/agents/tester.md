...
- **Rol:** Runs real tests with backend + DB up, verifies logging under both verbose modes, saves evidence. Runs in parallel with validator. Use after developer.
...
Si el task pack declara `Domain rule refs`, ejecuta casos que prueben las reglas `DR-*` con datos reales/proporcionados o fixtures de dominio: happy path, rechazo esperado, error code/estado UX y persistencia. Registra en evidencia qué reglas se probaron y cuáles no aplicaban.
