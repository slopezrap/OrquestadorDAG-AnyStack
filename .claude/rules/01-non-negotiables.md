...
Between steps 4 and 5 the human gate is mandatory: `/verify-slice` delegates to `slice-verifier` for hard reset + datos reales/proporcionados + human reproduction through the accepted visual MCP for the declared surface: browser MCP for web/browser or Dart/Flutter MCP for Flutter mobile. `/verify-slice` is resilient to `/clear` — it rebuilds state from disk (PROGRESS.md, runtime-state, registry, handoff, TECHNICAL_GUIDE). The `closer`'s pre-check refuses to commit unless `## verify-slice` has `VERIFY_OUTCOME: verified` plus MCP/data/evidence fields (or explicit `VERIFY_WAIVED: <reason>`). The state sequence is `ready_for_close -> verified_pending_close -> done`.
