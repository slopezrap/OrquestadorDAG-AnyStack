...
**Comandos hermanos**: `/verify-slice` (gate humano/recovery sin cierre), `/closer` (cierre Git manual tras verify), `/revise-slice <TASK_ID>` (corrección) y `/slice-maintain clean|compact` (limpieza + compactación). Orden recomendado: `/next-slice` implementa y verifica automáticamente → si queda `verified_pending_close`, el usuario revisa el resumen y ejecuta `/closer <TASK_ID>` → `/slice-maintain clean` → `/clear` → `/next-slice`.
...
**Gate adicional — journey verification**: si al verificar esta slice todos los demás slices de un journey ya están `done`, el tramo automático de `/verify-slice` integrará el gate de journey en su §5.bis: ofrecerá verificar el journey inline (mismo entorno, mismos datos cargados, un solo gate humano) o dejarlo "aparte" para `/verify-journey <JID>` después. La detección se hace con `list_journey_closures.py`, no con `task_ids[-1]`, para soportar DAGs y matrices desordenadas. Si elige aparte, `/closer` emitirá `JOURNEY_PENDING_VERIFY` y el próximo `/next-slice` quedará bloqueado por el planner sólo para tasks de ese journey hasta que se resuelva.

**Este comando NO invoca closer.** El pipeline de `/next-slice` ya no se queda esperando menú en `tester pass`: si validator/tester están verdes, continúa automáticamente con el contrato de `/verify-slice` y termina en `verified_pending_close` o en un bloqueo verificable. El cierre Git queda reservado al comando manual `/closer <TASK_ID>`.
...
## Pipeline por slice (/next-slice implementa y verifica; closer NO se invoca aquí)
...
4. debugger                    [si tester falla O validator pide cambios → volver a paso 3; máx 3 ciclos]
   ── si ambos están verdes, /next-slice continúa automáticamente con verify ──
5. /verify-slice automático — este mismo comando continúa con hard reset, datos reales/proporcionados y reproducción humana-real. No spawnea closer.
6. /closer <TASK_ID> — lo invoca el usuario después del resumen verified; report + commit atómico + workflow Git configurado + limpieza segura.
...
**Cadena (este comando continúa automáticamente a verify si validator/tester están verdes; closer NO se invoca desde aquí):**
...
── Fin del pipeline de implementación. Si validator/tester están verdes, empieza automáticamente el gate humano-real. ──

5. **Verificación automática post-slice — sin menú intermedio**

Cuando `validator=approved` y `tester=pass`, no preguntes "verify/espera/closer". Continúa en el mismo comando con el contrato de `.claude/commands/verify-slice.md` desde el router de estado hasta el Paso 6, con estas reglas adicionales:

```text
TASK_ID: <TASK_ID>
CLAUDE_TASK_PACK: orchestrator-state/tasks/task-packs/<TASK_ID>.md
MODO DAG ACTIVO: production = explicit_dag.
Modo de entrada: auto_from_next_slice.
No spawnees closer.
No hagas commit.
No marques done.
```

Antes de llamar a `slice-verifier`, revisa el task pack: si declara `Domain rule refs`, pásalas literalmente al prompt de verificación y exige que el handoff final incluya `DOMAIN_RULES_VERIFIED` con esas reglas `DR-*` o una razón `not_applicable` específica. No basta con haber probado el endpoint/pantalla si la invariante de dominio no fue observada.
...
- Si devuelve `invoke_slice_verifier`, ejecuta `./scripts/init-verify-slice-handoff.sh <TASK_ID>` y spawnea `slice-verifier` con hard reset, datos reales/proporcionados, navegador MCP usable, logs front/back/DB, evidencia y tabla.
- Si devuelve `invoke_debugger_or_register_followup` o `invoke_debugger`, sigue la política de `/verify-slice`: debugger dentro del scope, luego `validator ‖ tester`, y vuelve a verificar.
- Si devuelve `invoke_closer`, interpreta eso como `ready_for_manual_closer`: valida handoff y termina mostrando `/closer <TASK_ID>` como siguiente acción humana. **No invoques closer.**
- Si devuelve `blocked`, muestra el blocker y no inventes FU de producto por problemas mecánicos.

Si la task toca UI, UX, journey, rutas, pantallas, VISUAL_CONTRACT_CHECK/visual contract, auth visible, navegación o `journey_refs`, ejecuta también `screen-journey-reviewer` igual que `/verify-slice` antes de dejar la slice lista para `/closer`.

Si esta slice cierra journey(s), usa `python3 -B -S .claude/bin/list_journey_closures.py <TASK_ID> --json` y aplica el §5.bis de `/verify-slice`: verifica inline si el usuario lo acepta, o deja `JOURNEY_PENDING_VERIFY` para que `/closer` lo emita y el DAG bloquee sólo ese journey.

Al terminar correctamente, el estado esperado es:

```text
validator approved + tester pass + VERIFY_OUTCOME: verified
NEXT_STATUS: verified_pending_close
CLOSER_ACTION: user_required
NEXT_ACTION: /closer <TASK_ID>
```
...
Cuando termine la reproducción humana-real, si todo está verified, te mostraré `/closer <TASK_ID>` como siguiente acción manual.
```
