...
**Comandos hermanos**: `/verify-slice`, `/next-slice`, `/closer`, `/slice-maintain`. Orden recomendado al cerrar la ÚLTIMA slice de un journey: `/next-slice` deja la slice en `verified_pending_close` mediante verificación automática → `/closer TASK_ID` → `/clear` → `/verify-journey JXXX` → `/slice-maintain clean` → `/next-slice` (siguiente journey).
