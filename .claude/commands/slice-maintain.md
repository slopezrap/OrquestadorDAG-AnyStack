...
- `.claude/rules/**`, `.claude/agents/**`, `.claude/skills/**`, `.claude/bin/**` (incluye `hook_*.py`), `.claude/commands/**` — `.claude/` es configuración estática; no la limpies desde runtime. Los instaladores/configuradores locales antiguos fueron eliminados del orquestador.
