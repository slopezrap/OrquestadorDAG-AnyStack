...
> - El `official-docs-researcher` corre antes del `developer` en CADA slice. Es quien resuelve la versión exacta al introducir la lib en `frontend dependency manifest` / `backend dependency manifest`.
...
> |---|---|---|---|---|---|---|---|---|---|---|---|---|
> | P02-S02-T001 | api | `POST /api/v1/{{resource}}` | Step 2.2 | P02-S01-T001 | J101 | — | `POST /api/v1/{{resource}}` | {{resource_table}} | §3.1, journey section | §6.2, §10.3 | schema + use case + repo + integration test + curl + logs | `{{backend_test_cmd}}` + curl |
