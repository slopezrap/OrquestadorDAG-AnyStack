...
> - El `official-docs-researcher` se invoca solo cuando el `planner` marque `NEEDS_OFFICIAL_DOCS: yes` o cuando la slice introduzca/verifique una librería/API externa no confirmada. En ese caso resuelve la versión exacta al introducir la lib en el manifiesto de dependencias correcto.
...
Antes de entregar `*_TECHNICAL_GUIDE.md`, ChatGPT debe comprobar que la lógica declarada en `instrucciones.md` tiene aterrizaje técnico verificable.

- [ ] Cada `AL-*` tiene servicio/use case, endpoint/job/handler, DTO/schema, transacción/idempotencia y test.
- [ ] Cada `CORE-*` tiene módulo/función, tipos de entrada/salida, estrategia de reproducibilidad, fixture/dataset y test determinista `EVAL-*`.
- [ ] Cada `DR-*` tiene enforcement técnico: backend, DB constraint, frontend guard, test o combinación explícita.
- [ ] Cada `AUTH-*` tiene enforcement backend-side y comportamiento UI asociado; la UI no es frontera de seguridad.
- [ ] Cada `STATE-*` tiene storage, transición, evento/audit y test de transición prohibida.
- [ ] Cada `ERR-*` tiene código/status, comportamiento de rollback/retry, copy visible si aplica y logs limpios.
- [ ] Cada `INT-*` tiene client/wrapper, timeout, retry, idempotency key, redacción de logs y failure mode.
- [ ] Cada `DATA-*` tiene tabla/modelo, constraints, ownership, retención/borrado y datos reales/proporcionados si se verifica.
- [ ] Cada `OBS-*` tiene evento, campos, correlation id/request id y ubicación de persistencia/log.
- [ ] Cada pantalla/ruta consume endpoints reales declarados y mapea estados UI obligatorios.
- [ ] El `Verification Data Contract` cubre todos los journeys y `CORE-*` que se deben verificar.
