# Browser MCP verification policy

`/verify-slice` requires one usable browser MCP for human-real verification. Listed tools are not enough: `slice-verifier` must make a small health call before relying on a server.

## Accepted MCPs and priority
...
Do not use Playwright/browser-use as the normal `/verify-slice` gate. They can be project-specific support tools, but they do not replace the accepted browser MCP proof unless the user explicitly waives human browser verification.
...
If all browser MCP candidates are listed but unusable, `slice-verifier` must append a final blocked `## verify-slice` section with:
...
This is a mechanical blocker, not a product follow-up.

## Lifecycle placement in DAG mode

Do not kill or restart browser MCPs from `/next-wave`. `next-wave` can run while other terminals/worktrees are still using visible browser sessions, so process-level MCP cleanup there is too aggressive.
...
- `/verify-slice` / `slice-verifier`: perform browser MCP preflight, diagnose Chrome DevTools profile locks with `scripts/chrome-mcp-doctor.sh`, and block cleanly if no accepted MCP is usable.
- Manual recovery: close/restart the affected MCP/browser session yourself, then rerun `/verify-slice <TASK_ID>`. The orchestrator should not kill Chrome or MCP server processes automatically.

This avoids one slice breaking another slice's MFA/login/browser verification in a parallel DAG run.
