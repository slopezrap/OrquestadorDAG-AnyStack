...
> Estas reglas son canónicas dentro de `instrucciones.md`; el Technical Guide las aterriza y el Coverage Registry las referencia con `Domain rule refs`.

| Rule ID | Regla | Tipo | Aplica a | Fuente/razón | Error esperado | Verificación |
|---|---|---|---|---|---|---|
| DR-001 | Todo {app.domain} debe persistirse con título no vacío y owner/session válido | invariant | {app.entity}, POST {app.endpoint_base} | F1 / journey principal | 400 DOMAIN_VALIDATION_FAILED | test de API + verify-slice con dato real/proporcionado |
| DR-002 | Las modificaciones de {app.domain} solo son válidas sobre registros existentes y permitidos | state/authorization | {app.entity}, PATCH/GET {app.endpoint_base}/{{id}} | F2 / journey de revisión | 403 DOMAIN_FORBIDDEN o 404 DOMAIN_NOT_FOUND | fixture real/proporcionada para owner y registro inexistente |

## 2.6 Application Logic Contract

| AL ID | Caso de uso | Trigger | Actor | Preconditions | Pasos internos | Outputs | Refs |
|---|---|---|---|---|---|---|---|
| AL-001 | Crear {app.domain} | submit de pantalla principal | operator | sesión válida | validar input; aplicar DR-001; persistir {app.entity}; devolver DTO | {app.domain} persistido | DR-001, CORE-001, AUTH-001, STATE-001, ERR-001 |
| AL-002 | Revisar {app.domain} | abrir/actualizar detalle | operator | registro existente permitido | cargar registro; aplicar DR-002; devolver o actualizar estado | detalle consistente | DR-002, AUTH-001, STATE-001, ERR-002 |

## 2.7 Core Logic Contract

| Core ID | Nombre | Propósito | Inputs | Parámetros | Algoritmo / pasos | Outputs | Verificación mínima |
|---|---|---|---|---|---|---|---|
| CORE-001 | {app.domain} workflow core | validar y normalizar el recurso central | payload {app.domain} | reglas de validación | normalizar; validar; producir DTO persistible | DTO validado | test determinista de creación y lectura |

## 2.8 Permission Logic Contract

| Auth ID | Actor | Resource | Action | Allowed when | Denied when | Error |
|---|---|---|---|---|---|---|
| AUTH-001 | operator | {app.entity} | read/write | owner/session válido | owner inválido o sesión ausente | 401/403 |
...
|---|---|---|---|---|---|
| STATE-001 | {app.entity} | draft, active, archived | draft->active, active->archived | archived->draft | fixture de transición válida e inválida |

## 2.10 Failure Logic Contract

| Error ID | Scenario | Expected behavior | User message | State change | Retry? | Applies to |
|---|---|---|---|---|---|---|
| ERR-001 | input inválido | rechazar sin persistir | Corrige los datos | none | no | AL-001 |
| ERR-002 | registro no permitido | 403/404 sin filtrar datos | No disponible | none | no | AL-002 |

## 2.11 Data and Observability Logic

| ID | Tipo | Qué queda definido | Evidencia | Applies to |
|---|---|---|---|---|
| DATA-001 | data lifecycle | {app.entity} se crea por AL-001 y se actualiza por AL-002 | fila persistida | AL-001, AL-002 |
| OBS-001 | audit/trace | created/updated con actor y request id | audit log | AL-001, AL-002 |
| EVAL-001 | evaluation | creación y lectura producen DTO esperado | test determinista | CORE-001 |

## 2.12 Architecture Blueprint Contract

| A42 ID | arc42 section | Decisión / contenido concreto | Impacto en slices | Verify |
|---|---|---|---|---|
| A42-01 | Introduction and Goals | {app.name} debe cerrar journeys visibles con datos persistidos | all | golden e2e |
| A42-03 | Context and Scope | frontend, API y DB local son límites del sistema golden | API/front/db slices | wiring contract |
| A42-04 | Solution Strategy | screen/journey lane con API y DB conectadas | all | next-wave + verify |
| A42-05 | Building Block View | frontend page, API router, service y repository | all | task-packs |
| A42-06 | Runtime View | usuario crea y revisa {app.domain} de punta a punta | journey slices | verify journey |
| A42-10 | Quality Requirements | datos reales/proporcionados y logs limpios | all | golden e2e |
...
|---|---|---|---|---|---|---|---|
| DR-001 | backend + db + frontend | POST {app.endpoint_base} | title not null, owner/session required | Create{app.domain.title()}UseCase | error_validation | API integration fixture + provided data | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| DR-002 | backend + frontend | {api_rows[-1][0] + ' ' + api_rows[-1][1] if api_rows else 'GET ' + app.endpoint_base} | owner/status visibility check | ReadOrUpdate{app.domain.title()}UseCase | permission_denied / not_found | fixture for forbidden and missing rows | {api_rows[-1][-1] if api_rows else 'P01-S01-T001'} |
...
|---|---|---|---|---|---|---|
| AL-001 | Create{app.domain.title()}UseCase | POST {app.endpoint_base} | request/response DTO | one create per request id | create integration test | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| CORE-001 | {app.domain.title()}CoreService | validate_and_normalize | typed payload/result | deterministic pure function | unit test fixture | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
...
|---|---|---|---|---|---|---|
| AUTH-001 | auth middleware + repository filter | all {app.domain} routes | owner/session check | permission_denied/not_found | forbidden fixture | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| STATE-001 | use case | create/update route | status column | state-specific UI | state fixture | {api_rows[-1][-1] if api_rows else 'P01-S01-T001'} |
| ERR-001 | API schema validation | POST {app.endpoint_base} | no write on invalid input | inline validation error | invalid fixture | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| DATA-001 | repository | {app.endpoint_base} | {app.entity} table | persisted row visible | DB assertion | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| OBS-001 | audit logger | create/update routes | audit event | support trace only | log assertion | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
| EVAL-001 | test harness | core service | fixture artifact | deterministic result | unit test | {api_rows[0][-1] if api_rows else 'P01-S01-T001'} |
...
def checklist(app: App, rows: list[list[str]], phases: list[tuple[str, str, list[str]]]) -> str:
    ...
    for row in rows:
        base = list(row)
        if len(base) == 19:
            refs = "DR-001,DR-002" if row[11] not in {"—", "-", ""} or row[1] in {"api", "frontend", "flutter", "journey"} else "DR-001"
            base.append(refs)
        if len(base) == 20:
            is_visible = base[11] not in {"—", "-", ""} or base[1] in {"api", "frontend", "flutter", "journey"}
            base.extend([
                "A42-04,A42-05,A42-06" if is_visible else "A42-01",
                "AL-001" if is_visible else "—",
                "CORE-001" if base[1] in {"api", "ai", "frontend", "flutter", "journey"} else "—",
                "AUTH-001" if is_visible else "—",
                "STATE-001" if is_visible else "—",
                "ERR-001" if is_visible else "—",
                "INT-001" if base[1] in {"api", "ai"} else "—",
                "UI-001" if base[1] in {"frontend", "flutter", "journey"} else "—",
                "DATA-001" if base[14] not in {"—", "-", ""} else "—",
                "OBS-001" if is_visible else "—",
                "EVAL-001" if base[1] in {"api", "ai"} else "—",
            ])
