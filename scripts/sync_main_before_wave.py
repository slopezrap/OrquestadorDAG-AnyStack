...
def sync(root: Path, remote: str, main_branch: str, *, apply: bool, quiet: bool, strict: bool = False) -> int:
    ...
    if branch != main_branch:
        switch_guard = call_runtime_guard(root, "backup")
        switch_backup = str(switch_guard.get("backup_dir") or "")
        if not switch_guard.get("ok", False):
            say("sync-main-before-wave: cannot switch canonical root to main; non-runtime dirty files would be hidden", quiet, err=True)
            for item in switch_guard.get("non_runtime", []) or []:
                if isinstance(item, dict):
                    say(f"DIRTY_NON_RUNTIME: {item.get('code','')} {item.get('path','')}", quiet, err=True)
            return 3
        if not apply:
            say(f"sync-main-before-wave: would switch canonical root from '{branch or 'DETACHED'}' to '{main_branch}'", quiet)
            return 0
        switch = run(["git", "switch", main_branch], cwd=root, timeout=120)
        if switch.returncode != 0:
            checkout = run(["git", "checkout", main_branch], cwd=root, timeout=120)
            if checkout.returncode != 0:
                if switch_backup:
                    call_runtime_guard(root, "restore", switch_backup)
                say(
                    f"sync-main-before-wave: failed to switch to {main_branch}; "
                    f"{checkout.stderr.strip() or switch.stderr.strip() or checkout.stdout.strip() or switch.stdout.strip()}",
                    quiet,
                    err=True,
                )
                return 3
        if switch_backup:
            restore = call_runtime_guard(root, "restore", switch_backup)
            if not restore.get("ok", False):
                say(f"sync-main-before-wave: runtime restore incomplete after branch switch backup={switch_backup}", quiet, err=True)
                return 3
        say(f"sync-main-before-wave: switched canonical root from '{branch or 'DETACHED'}' to '{main_branch}'", quiet)
    ...
    try:
        ...
        if remote_sha == base:
            say(
                ...
                "using local main for DAG wave. Push when convenient.",
                quiet,
                err=bool(strict),
            )
            return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--strict", action="store_true", help="make next-wave wrapper fail on non-recoverable sync issues")
    ...
    try:
        ...
        return 0
    return sync(root, args.remote, args.main, apply=bool(args.apply), quiet=bool(args.quiet), strict=bool(args.strict))
